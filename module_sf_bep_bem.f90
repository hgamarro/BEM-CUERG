...
MODULE module_sf_bep_bem
 ...
 USE module_sf_bem
      ...
      parameter(rcp_u=r/cp_u,sigma=5.67e-08,p0=1.e+5,latent=2.45e+06)
   ...
   REAL, INTENT(IN )::   DT      
	 ...
	 enddo
      ...
      subroutine BEP1D(iurb,kms,kme,kts,kte,z,dt,ua,va,pt,da,pr,pt0,   &  
                      ...
                      sfvlev,lfvlev,twlev,tglev,tflev,sfwin,ix,iy,lp_urb)                             
	 ...
	 tmp_u(iz)=pt_u(iz)*(pr_u(iz)/p0)**(rcp_u) 
        ...
        do ibui=1,nbui
         ...
         lfbui(ibui)=0
           ...
           do iz=1,nz_um
             ...
             if(iurb.eq.1) then
                 sfbui(ibui)=sfbui(ibui)+sflev1D(iz)
                 lfbui(ibui)=lfbui(ibui)+lflev1D(iz)
                 sflev(iz,ibui)=0
                 lflev(iz,ibui)=0
             else
           ...
           if(iurb.eq.1.and.sfbui(ibui).gt.0) then
               call cooling_tower(ibui,nlev,tmp_u,pr_u,qv_u,sfbui,lfbui,sflev,lflev)
           endif
      ...
      call buildings(iurb,ndu,nzu,z0,ua_u,va_u,                               & 
                     ...
                     sfwin,pb,bs_u,dz_u,sflev,lflev,sfvlev,lfvlev,tvb_ac,lp_urb)  
      ...
      do id=1,nd
        ...
        if(lp_urb.le.0.29) then
        cdrag=3.32*(lp_urb)**0.47
        else
        cdrag=1.85
        endif
...
'singular matrix in gaussj')
         ...
         if(a(icol,icol).eq.0) CALL wrf_error_fatal3("<stdin>",3331,&
'singular matrix in gaussj')
...
subroutine cooling_tower(ibui,nlev,tmp_u,pr_u,qv_u,sfbui,lfbui,                     &
                          sflev,lflev)
  ...
  implicit none
      ...
      real tb,tu,a,b,taon,taon1,ten,c,diff,z,eso,qvos,rh
      real prr    
      real es    
      real qvs   
      real qvi   
...
tai=tmp_u(nlev(ibui))-273.15
qvi=qv_u(nlev(ibui))
prr=0.01*pr_u(nlev(ibui))




      eso=6.11*(10**(tai*7.5/(237.7+tai)))
      qvos=0.622*eso/(prr-0.378*eso)
      rh=100*qvi/qvos
      two=tai*atan(0.151977*((rh+8.313659)**0.5))+atan(tai+rh)-atan(rh-1.676331)+0.00391838*(rh**(1.5))*atan(0.023101*rh)-4.686035



cmin=sfbui(ibui)/((trefi-two)*eff)

twi=two+(sfbui(ibui)/cmin)




      hai= cp*tai + qvi*(cpw*tai + latent)

      es=6.11*(10**(twi*7.5/(237.7+twi)))
      qvs=0.62197*es/(prr-0.378*es)
      hso= cp*twi + qvs*(cpw*twi + latent)

      ma=sfbui(ibui)/(eff*(hso-hai))
...
z=6.11*10**(7.5*taon/(237.7+taon))

a=taon*z*(0.62197*cpw-cp*0.378) + z*(0.6197*latent+c*0.378) + taon*prr*cp - c*prr




b=(0.62197*cpw-cp)*z+taon*((z/6.11*2.3026*((7.5/(237.7+taon))-(7.5*taon/((237.7+taon)**2)))))+(6.11*(0.6197*latent+c))*(z/6.11*2.3026*((7.5/(237.7+taon))-(7.5*taon/((237.7+taon)**2))))+(prr*cp)
...
es=6.11*(10**(tao*7.5/(237.7+tao)))
qvo=0.62197*es/(prr-es*0.378)

      lflev(nlev(ibui),ibui)=ma*latent*(qvo-qvi)+lfbui(ibui)
      sflev(nlev(ibui),ibui)=ma*cp*(tao-tai)
